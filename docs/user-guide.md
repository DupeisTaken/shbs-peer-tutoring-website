...
The combined website supports intake, matching, attendance, hours, interviews, support and reviewed management changes. The operator must configure real email delivery and publish the current school policies before opening public intake. A repository policy revision is not automatically a published school policy.
...
Read the [tutor policy](../prisma/policies/tutor-policy.en.md) or [中文辅导员政策](../prisma/policies/tutor-policy.zh.md).

## Crew

Active crew members use **Patrol** to record what they actually observe in the relevant rooms. Check the date and observations, then submit the patrol. Report mistakes to management with enough context to identify the record. Historical patrol corrections require a reason and audit evidence; coordinator corrections wait for approval.

Crew access depends on membership and the program’s crew module. A crew-only account does not automatically receive tutor assignments or management powers. Use the crew participation request controls when available and check their pending/resolved state before assuming membership has changed.
...
| Hours look different from ordinary rounding | Compare the recorded attendance and the [confirmed custom rounding examples](../prisma/policies/tutor-policy.en.md#service-hours) |
