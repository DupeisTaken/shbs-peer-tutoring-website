...
<div class="layout"><aside>${nav}<div class="aside-note">Program reference<br><strong>9 September 2026</strong><br>Code merged · launch setup pending</div></aside><main id="report"><div class="edition"><span>SHBS / Documentation</span><span>${name === "user-guide" ? "01 · Participation & roles" : "02 · Engineering & operations"}</span></div><article>${model.html}</article><footer>Maintained in the repository · <a href="${repository}${source}">View Markdown source</a><br>Local report navigation works offline. Repository and policy links require internet access.</footer></main></div>
</body></html>
`;
}
