...
beforeEach(async () => {
  ...
  if (
    ...
    throw new Error(
      "Tutor detail tests require isolated local shbs_shipping_test",
    );
  ...
  await db.user.createMany({
    data: [
      { id: "detail-reviewer", email: "reviewer@example.test", role: "HEAD" },
      {
        ...
        role: "STUDENT",
        tutorId: "detail-tutor",
        tuteeMember: true,
      },
    ],
  });
  await db.courseGroup.create({
    data: { id: "detail-group", name: "Computer Science", rank: 1 },
  });
    ...
    data: [
      {
        ...
        levelId: "detail-standard",
      },
      {
        id: "detail-advanced",
        name: "AP Computer Science A",
        baseName: "Computer Science A",
        groupId: "detail-group",
        levelId: "detail-ap",
      },
    ],
  ...
  await db.tutorQualification.create({
    data: {
      tutorId: "detail-tutor",
      subjectId: "detail-advanced",
      status: "APPROVED",
      approvedById: "detail-reviewer",
    },
  });
  await db.qualificationGrant.createMany({
    data: ["detail-intro", "detail-advanced"].map((subjectId) => ({
      tutorId: "detail-tutor",
      sourceSubjectId: "detail-advanced",
      subjectId,
    })),
  });
  await db.tutorSubjectWillingness.create({
    data: {
      tutorId: "detail-tutor",
      subjectId: "detail-intro",
      willing: false,
    },
  });
  await db.policyDocument.createMany({
    data: ["tutee-policy", "tutor-policy"].map((slug) => ({
      slug,
      locale: "en",
      title: "Synthetic policy",
      body: "Original synthetic consent",
      version: "1",
    })),
  });
});
    ...
    expect(result.groups[0]?.subjects).toMatchObject([
      {
        ...
        qualified: true,
        willing: false,
        inheritedFrom: [{ id: "detail-advanced" }],
      },
      { id: "detail-advanced", qualified: true, willing: null },
    ]);
    expect(result).not.toHaveProperty("passwordHash");
    expect(
      await db.user.findUniqueOrThrow({ where: { id: "detail-user" } }),
    ).toEqual(before);
...
it.each(["VIEWER", "TUTOR", "STUDENT", "CREW"] as const)(
  "%s cannot inspect another tutor even with a stale management session",
  async (role) => {
    await db.user.update({ where: { id: "detail-reviewer" }, data: { role } });
    await expect(
      caller().tutorDetails.get({ tutorId: "detail-tutor" }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      caller().student.acceptanceRecords({ userId: "detail-user" }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  },
);
...
it("preserves inherited approval after reorder but ignores grants from rejected sources", async () => {
  ...
  const result = await caller().tutorDetails.get({ tutorId: "detail-tutor" });
  expect(result.groups[0]?.subjects.map((subject) => subject.id)).toEqual([
    "detail-advanced",
    "detail-intro",
  ]);
  expect(result.groups[0]?.subjects.every((subject) => subject.qualified)).toBe(
    true,
  );
  await db.tutorQualification.update({
    where: {
      tutorId_subjectId: {
        tutorId: "detail-tutor",
        subjectId: "detail-advanced",
  ...
  expect(
    (
      await caller().tutorDetails.get({ tutorId: "detail-tutor" })
    ).groups[0]?.subjects.every((subject) => !subject.qualified),
  ).toBe(true);
});

it("returns unlinked and revoked records truthfully and rejects unknown tutors", async () => {
  expect(
    await caller().tutorDetails.get({ tutorId: "detail-unlinked" }),
  ).toMatchObject({ userId: null, badges: [] });
  ...
  await expect(
    caller().tutorDetails.get({ tutorId: "missing" }),
...
it("uses linked account identity for accepted/unaccepted tutee policy and preserves historical words", async () => {
  ...
  const unaccepted = await caller().student.acceptanceRecords({
    userId: detail.userId!,
  });
  expect(
    unaccepted.current.find((policy) => policy.slug === "tutee-policy")
      ?.acceptedAt,
  ).toBeNull();
  ...
  await db.policyAcceptance.create({
    data: {
      ...
      signature: "Synthetic Tutor",
    },
  });
  expect(
    (
      await caller().student.acceptanceRecords({ userId: detail.userId! })
    ).current.find((item) => item.slug === "tutee-policy")?.acceptedAt,
  ).toBeInstanceOf(Date);
  await db.policyDocument.updateMany({
    where: { slug: "tutee-policy" },
    data: { body: "Revised synthetic consent", version: "2" },
  });
  const changed = await caller().student.acceptanceRecords({
    userId: detail.userId!,
  });
  expect(
    changed.current.find((item) => item.slug === "tutee-policy")?.acceptedAt,
  ).toBeNull();
  expect(changed.rows[0]?.documents[0]).toMatchObject({
    version: "1",
    body: "Original synthetic consent",
  });
});
