...
it("adds field defaults without fabricating historical application consent or changing answers", async () => {
  ...
  if (url.pathname !== "/shbs_shipping_test")
  ...
  try {
    ...
    await client.query(
      "BEGIN; CREATE SCHEMA signup_fields_migration_test; SET LOCAL search_path TO signup_fields_migration_test",
    );
    await client.query(`
    ...
    await client.query(
      readFileSync(
        new URL(
          "../../prisma/migrations/20260919040000_signup_field_settings/migration.sql",
          import.meta.url,
        ),
        "utf8",
      ),
    ...
    expect(
      (await client.query('SELECT "signupFields" FROM "ProgramSettings"')).rows,
    ).toEqual([{ signupFields: {} }]);
    expect(
      (await client.query('SELECT * FROM "TutorApplication"')).rows,
    ).toEqual([
      {
        id: "historical",
        preferredContact: "Keep original answer",
        policyRevision: null,
        policySnapshot: null,
    ...
    await expect(
      client.query('UPDATE "ProgramSettings" SET "signupFields" = \'[]\''),
    ).rejects.toMatchObject({ code: "23514" });
    await client.query("ROLLBACK TO SAVEPOINT invalid_config");
    await expect(
      client.query(
        'UPDATE "TutorApplication" SET "policyRevision" = \'invented\'',
      ),
    ).rejects.toMatchObject({ code: "23514" });
  } finally {
