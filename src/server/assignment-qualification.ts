...
export function overrideTarget(operation: string, input: unknown) {
  return createHash("sha256").update(`${operation}:${assignmentIdentity(input)}`).digest("hex");
}
...
export async function assignmentMismatches(tx: TransactionDb, operation: string, value: unknown) {
  ...
  if (operation === "admin.assignTuteeToTutor" && !input.subject && !input.subjectId) {
    ...
    selections = [{ tutorId: input.tutorId!, subjectId: tutee?.firstChoiceId ?? undefined, subject: undefined }];
  }
  const mismatches: { tutorId: string; tutorName: string; subjectId: string; subjectName: string }[] = [];
...
export async function prepareAssignmentOverride(tx: TransactionDb, userId: string, operation: string, input: unknown) {
  ...
  await tx.studentActionConfirmation.deleteMany({ where: { userId, action: ACTION, usedAt: null } });
  if (!mismatches.length) return { mismatches, ticket: null };
  const now = Date.now();
  const ticket = await tx.studentActionConfirmation.create({ data: {
    ...
    userId, action: ACTION, target: overrideTarget(operation, { input, mismatches }),
    readyAt: new Date(now + 3000), expiresAt: new Date(now + 5 * 60_000),
...
export async function enforceAssignmentQualification(tx: TransactionDb, userId: string, operation: string, input: unknown) {
  if (!isAssignmentOperation(operation)) return;
  const mismatches = await assignmentMismatches(tx, operation, input);
  if (!mismatches.length) return;
  const ticket = inputShape.parse(input).overrideTicket;
  const now = new Date();
  const claimed = ticket ? await tx.studentActionConfirmation.updateMany({ where: {
    id: ticket, userId, action: ACTION, target: overrideTarget(operation, { input, mismatches }),
    usedAt: null, readyAt: { lte: now }, expiresAt: { gt: now },
  }, data: { usedAt: now } }) : { count: 0 };
  if (!claimed.count) throw new TRPCError({ code: "PRECONDITION_FAILED", message: "This tutor has no approved qualification for the selected subject. Open the warning, wait three seconds, and explicitly confirm this assignment." });
}
