...
export const tutorRouter = createTRPCRouter({
  ...
  decideInterview: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await assertFeatureEnabled(ctx.db, "INTERVIEWS");
      // Review retains the original chair check, but records the actual admin as decision maker.
      const reviewId = approvalScope.getStore();
      let chairTutorId = ctx.session.tutorId;
      if (reviewId && ["ADMIN", "HEAD"].includes(ctx.session.role)) {
        const request = await ctx.db.approvalRequest.findUniqueOrThrow({
          where: { id: reviewId },
        });
      ...
      if (!chairTutorId)
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "An assigned interview chair is required.",
        });
      const assignment = await ctx.db.interviewAssignment.findUnique({
        where: {
          applicationId_tutorId: {
      ...
      if (ctx.session.role === "COORDINATOR") {
