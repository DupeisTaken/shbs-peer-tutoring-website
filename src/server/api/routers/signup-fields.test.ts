...
const caller = (role: Session["role"] | null = null) =>
  createCaller({
    db,
    headers: new Headers(),
    session: role
...
const change = async (
  ...
  return caller("HEAD").program.setSignupField({
    form,
    field,
    state,
    expectedState: current.fields[form][field]!,
  });
};
const tutorInput = async () => ({
  name: "Applicant",
  email: "applicant@example.test",
  preferredContact: "Email",
  agreed: true as const,
  policyRevision: (await currentPolicy(db, "tutor-policy")).revision,
  subjects: [{ subjectId: "fields-math" }],
});
const tuteeInput = async () => ({
  englishName: "Student",
  email: "student@example.test",
  preferredContact: "Email",
  agreed: true as const,
  policyRevision: (await currentPolicy(db, "tutee-policy")).revision,
  firstChoiceId: "fields-math",
...
beforeEach(async () => {
  ...
  for (const slug of ["tutor-policy", "tutee-policy"])
    await db.policyDocument.create({
      data: { slug, locale: "en", title: "Policy", body: "Synthetic policy" },
    });
  ...
  await db.subject.createMany({
    data: [
      { id: "fields-math", name: "Math" },
      { id: "fields-science", name: "Science" },
    ],
  });
...
it("restricts writes to Head, rejects essential/custom fields, and guards stale changes", async () => {
  ...
  for (const role of [
    ...
    await expect(
      caller(role).program.setSignupField(input),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  await expect(caller().program.setSignupField(input)).rejects.toMatchObject({
    code: "UNAUTHORIZED",
  });
  for (const form of ["tutee", "tutor"] as const)
    for (const field of [
      ...SIGNUP_FIELDS[form].filter((f) => f.locked).map((f) => f.key),
      "custom",
    ])
      await expect(
  ...
  await expect(
    caller("HEAD").program.setSignupField({ ...input, state: "hidden" }),
...
it("enforces current tutee requirements, strips hidden answers, and preserves earlier submissions", async () => {
  ...
  await caller().tutee.submitSurvey({
    ...
    preferredContact: "discard",
    slotIds: ["invalid-hidden-slot"],
    phone: "discard",
    secondChoiceId: "fields-science",
    signatureName: "",
  });
  ...
  expect(
    (await db.studentSurvey.findUniqueOrThrow({ where: { id: original.id } }))
      .payload,
  ...
  for (const invalid of [
    ...
    await expect(
      caller().tutee.submitSurvey({ ...input, ...invalid } as typeof input),
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
});
...
it("enforces tutor consent and positional additional subjects while preserving historical evidence", async () => {
  ...
  for (const invalid of [
    ...
    await expect(
      caller().application.submit({ ...input, ...invalid } as typeof input),
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  await expect(
    caller().application.submit({ ...input, policyRevision: "stale" }),
  ).rejects.toMatchObject({ code: "CONFLICT" });
  await caller().application.submit(input);
  const original = await db.tutorApplication.findFirstOrThrow();
  expect(original.policySnapshot).toEqual(
    (await currentPolicy(db, "tutor-policy")).documents,
  );
  ...
  await expect(caller().application.submit(input)).rejects.toMatchObject({
    code: "BAD_REQUEST",
  });
  await caller().application.submit({
    ...
    preferredContact: "",
    subjects: [
      { subjectId: "fields-math" },
      { subjectId: "invalid-hidden-subject" },
      { subjectId: "fields-science" },
    ],
  });
  const saved = await db.tutorApplication.findFirstOrThrow({
    where: { email: "new@example.test" },
    include: { subjectIntents: true },
  });
  expect(saved.preferredContact).toBeNull();
  expect(saved.subjectIntents.map((row) => row.subjectId).sort()).toEqual([
    "fields-math",
    "fields-science",
  ]);
  expect(
    await db.tutorApplication.findUniqueOrThrow({ where: { id: original.id } }),
  ).toEqual(original);
...
it("requires explicit yes/no answers and only applicable qualification details", async () => {
  ...
  await caller().application.submit({
    ...input,
    subjects: [{ subjectId: "fields-math", taken: false }],
  });
  await expect(
    caller().application.submit({
      ...input,
      subjects: [{ subjectId: "fields-math", taken: true }],
    }),
  ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  await change("tutor", "taken", "hidden");
  await caller().application.submit({
    ...input,
    subjects: [{ subjectId: "fields-math", taken: true, grade: "discard" }],
  });
  expect(await db.applicationSubjectIntent.findMany()).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ taken: false, grade: null }),
    ]),
  );
});
...
it("requires AP details only for AP subjects and keeps primary signup independent of secondary-email binding", async () => {
  ...
  const level = await db.subjectLevel.create({
    data: { name: "AP", apScored: true },
  });
  await db.subject.update({
    where: { id: "fields-science" },
  ...
  await db.programSettings.update({
    where: { id: "program" },
    data: { secondaryEmailBindingEnabled: false },
  });
  expect(
    (await caller("HEAD").program.signupFieldSettings())
  ...
  await expect(
    caller().application.submit({
      ...input,
      subjects: [{ subjectId: "fields-science" }],
    }),
  ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  await caller().application.submit({
    ...input,
    subjects: [{ subjectId: "fields-science", hasApScore: false }],
  });
  await expect(
    caller().application.submit({
      ...input,
      subjects: [{ subjectId: "fields-science", hasApScore: true }],
    }),
  ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  await caller().application.submit({
    ...input,
    subjects: [{ subjectId: "fields-science", hasApScore: true, apScore: "5" }],
  });
