...
import {
  enforceAssignmentQualification,
  overrideTarget,
  prepareAssignmentOverride,
  ...
  studentActionConfirmation: {
    ...
    create: vi.fn(
      async ({
        ...
        data: Omit<NonNullable<typeof ticket>, "id" | "usedAt">;
      }) => {
        ticket = { ...data, id: "ticket", usedAt: null };
        return ticket;
      },
    ),
    updateMany: vi.fn(
      async ({
        ...
        if (
          ...
          ticket.target !== where.target ||
          ticket.action !== where.action ||
          ticket.readyAt > where.readyAt.lte ||
...
it("requires explicit evidence at 2999ms and accepts it exactly at 3000ms only once", async () => {
  ...
  await expect(
    enforceAssignmentQualification(tx, "actor", operation, input),
...
it("rejects another actor, changed assignment, or expired evidence", async () => {
  ...
  await expect(
    enforceAssignmentQualification(tx, "reviewer", operation, {
      ...value,
      overrideTicket: "ticket",
    }),
  ).rejects.toMatchObject({ code: "PRECONDITION_FAILED" });
  await expect(
    enforceAssignmentQualification(tx, "actor", operation, {
      ...value,
      timeSlotId: "changed",
      overrideTicket: "ticket",
    }),
  ...
  expect(overrideTarget(operation, value)).not.toBe(
    overrideTarget("admin.updatePairing", value),
  );
});
it("reopening restarts the delay and eligibility uses only approved concrete grants", async () => {
  ...
  await expect(
    enforceAssignmentQualification(tx, "actor", operation, value),
  ).resolves.toBeUndefined();
