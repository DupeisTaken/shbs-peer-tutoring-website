...
const request = (subjectId = "history-ap", id = "applicant") =>
  caller(id).qualificationApplication.submit({
    subjectId,
    reason: "Synthetic qualification evidence",
  });
async function decide(id: string, accept = true, reviewer = "admin") {
  const app = await db.tutorApplication.findUniqueOrThrow({ where: { id } });
  return caller(reviewer).qualificationApplication.decide({
    id,
    accept,
...
async function panel(id: string, chair = "admin-tutor") {
  const app = await db.tutorApplication.findUniqueOrThrow({ where: { id } });
  return caller("admin").admin.assignInterviewers({
    applicationId: id,
    tutorIds: [chair, "panel-a-tutor", "panel-b-tutor"],
    headTutorId: chair,
    expectedUpdatedAt: app.updatedAt,
  });
}
...
beforeEach(async () => {
  ...
  if (new URL(process.env.DATABASE_URL!).pathname !== "/shbs_shipping_test")
    throw new Error("Use the serial shbs_shipping_test database");
  ...
  await db.courseGroup.createMany({
    data: [
      { id: "history", name: "History", rank: 0 },
      { id: "science", name: "Science", rank: 1 },
    ],
  });
  ...
  await db.subject.createMany({
    data: [
      ...["standard", "honors", "ap"].map((levelId) => ({
        id: `history-${levelId}`,
        name: `History ${levelId}`,
        baseName: "History",
        groupId: "history",
        levelId,
      })),
...
it("records a self-owned new subject request without granting eligibility or exposing other tutors' history", async () => {
  ...
  expect(
    ...
    requestedTutorId: "applicant-tutor",
    requestedSubjectId: "history-ap",
    status: "PENDING",
    promotedTutorId: null,
  });
  expect(await eligibleSubjectIds(db, "applicant-tutor")).toEqual([
    "science-standard",
  ]);
  expect(
    (await caller().qualificationApplication.mine()).requests.map(
      (row) => row.id,
    ),
  ).toEqual([id]);
  expect(
    (await caller("other").qualificationApplication.mine()).requests,
  ).toEqual([]);
  await expect(
    caller().qualificationApplication.submit({
...
it.each(["admin", "head"])(
  "lets %s approve directly with interviews enabled while preserving roles and prior grants",
  async (reviewer) => {
    const userBefore = await db.user.findUniqueOrThrow({
      where: { id: "applicant" },
    });
    const original = await db.tutorQualification.findUniqueOrThrow({
      where: {
        tutorId_subjectId: {
          tutorId: "applicant-tutor",
          subjectId: "science-standard",
        },
      },
    });
    const { id } = await request();
    ...
    expect(app).toMatchObject({
      status: "ACCEPTED",
      qualificationDecidedById: reviewer,
      promotedTutorId: null,
    });
    expect(
      qualificationSnapshot(app.qualificationSnapshot)
        .map((subject) => subject.id)
        .sort(),
    ).toEqual(["history-ap", "history-honors", "history-standard"]);
    expect((await eligibleSubjectIds(db, "applicant-tutor")).sort()).toEqual([
      "history-ap",
      "history-honors",
      "history-standard",
      "science-standard",
    ...
    expect(
      await db.user.findUniqueOrThrow({ where: { id: "applicant" } }),
    ).toEqual(userBefore);
    expect(
      await db.tutorQualification.findUniqueOrThrow({
        where: {
          tutorId_subjectId: {
            tutorId: "applicant-tutor",
            subjectId: "science-standard",
    ...
    expect(
      await db.tutorSubjectWillingness.count({
        where: { tutorId: "applicant-tutor" },
      }),
    ).toBe(0);
    expect(
...
it("classifies higher-level requests, snapshots missing intermediate offerings and survives later reordering", async () => {
  ...
  await db.subject.update({
    where: { id: "history-honors" },
    data: { active: false },
  });
  const { id } = await request();
  expect(
    (await db.tutorApplication.findUniqueOrThrow({ where: { id } })).type,
  ).toBe("HIGHER_LEVEL");
  await decide(id);
  const snapshot = (
    await db.tutorApplication.findUniqueOrThrow({ where: { id } })
  ...
  expect(
    (await caller().qualificationApplication.mine()).requests[0]
      ?.qualificationSnapshot,
  ).toEqual(snapshot);
});
...
it("serializes duplicate submissions and refuses already approved or inactive subjects", async () => {
  ...
  await expect(request("science-standard")).rejects.toMatchObject({
    code: "BAD_REQUEST",
  });
  ...
  await expect(request("history-honors")).rejects.toMatchObject({
    code: "BAD_REQUEST",
  });
});
...
it("rejects Coordinator decisions and legacy entry points before queuing any proposal", async () => {
  const { id } = await request();
  const app = await db.tutorApplication.findUniqueOrThrow({ where: { id } });
  await expect(decide(id, true, "coordinator")).rejects.toMatchObject({
    code: "FORBIDDEN",
  });
  await expect(
    caller("coordinator").admin.assignInterviewers({
      applicationId: id,
      tutorIds: ["coordinator-tutor", "panel-a-tutor", "panel-b-tutor"],
      headTutorId: "coordinator-tutor",
      expectedUpdatedAt: app.updatedAt,
    }),
  ).rejects.toMatchObject({ code: "FORBIDDEN" });
  for (const reviewer of ["coordinator", "admin", "head"]) {
    await expect(
      caller(reviewer).admin.setApplicationStatus({
        id,
        status: "ACCEPTED",
        expectedUpdatedAt: app.updatedAt,
      }),
    ...
    await expect(
      caller(reviewer).admin.deleteApplication({ id }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      caller(reviewer).tutor.decideInterview({
        applicationId: id,
        accept: true,
        comment: "Legacy bypass",
        expectedUpdatedAt: app.updatedAt,
      }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  }
...
it.each([true, false])(
  "retains interview records on decision %s and requires every vote, the majority and an Admin/Head chair",
  async (accept) => {
    const { id } = await request();
    await expect(panel(id, "coordinator-tutor")).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
    await panel(id);
    expect(await caller("panel-a").tutor.myInterviews()).toEqual(
    ...
    for (const voter of ["admin", "panel-a", "panel-b"])
      await caller(voter).tutor.castInterviewVote({
        applicationId: id,
        accept,
        comment: "Synthetic panel vote",
      });
    await expect(decide(id, !accept)).rejects.toMatchObject({
      code: "BAD_REQUEST",
    });
    await expect(decide(id, accept, "head")).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    ...
    expect(await db.interviewVote.count({ where: { applicationId: id } })).toBe(
      3,
    );
    expect(
      (await db.tutorApplication.findUniqueOrThrow({ where: { id } })).status,
    ).toBe(accept ? "ACCEPTED" : "REJECTED");
    expect(
...
it("preserves old qualifications after rejection, permits resubmission, and never archives the existing tutor", async () => {
  ...
  expect(
    (await db.tutor.findUniqueOrThrow({ where: { id: "applicant-tutor" } }))
      .status,
  ).toBe("ACTIVE");
...
it("blocks revoked and inactive tutor submissions, self-review and stale decisions", async () => {
  ...
  await expect(
    caller("head").qualificationApplication.decide({
      id,
      accept: true,
      comment: "Old form",
...
it("keeps direct review available without interviews but rolls back approval of removed offerings", async () => {
  ...
  expect(
    (await db.tutorApplication.findUniqueOrThrow({ where: { id: second.id } }))
      .status,
  ).toBe("PENDING");
