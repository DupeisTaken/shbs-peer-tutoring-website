...
import { assignmentIdentity, groupAssignmentTutors, isAssignableTutor } from "./assignment-qualification";
...
it("retains unlinked active roster tutors but excludes revoked and inactive memberships", () => {
  ...
  expect(isAssignableTutor({ status: "ACTIVE", user: { tutorAccessRevoked: false } })).toBe(true);
  expect(isAssignableTutor({ status: "ACTIVE", user: { tutorAccessRevoked: true } })).toBe(false);
