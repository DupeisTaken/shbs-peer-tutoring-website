...
function ApplicationCard({
  ...
  const courseNames = app.courseIntents.map((ci) => ci.course.name).join(", ") || "no courses";
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          <span className="muted hidden truncate text-xs sm:inline">
            {courseNames} · panel {app.interviewers.length}/{PANEL_SIZE}
            {app.votes.length > 0 ? ` · votes ${accepts}/${app.votes.length}` : ""}
          </span>
        </button>
        ...
        <div className="mt-3 border-t border-slate-100 pt-3">
          ...
          <div className="mt-4 border-t border-slate-100 pt-3">
            <p className="text-xs font-semibold tracking-wide text-slate-400 uppercase">
              Interview panel (choose {PANEL_SIZE}, mark one head)
            </p>
            {app.interviewAt && (
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              {!canAssign && !assign.isPending && (
                <span className="muted text-xs">Pick {PANEL_SIZE} tutors and a head.</span>
              )}
              ...
              {app.votes.length > 0 ? (
                <>
                  <p className="muted mt-1 text-sm">
                    {accepts} accept · {app.votes.length - accepts} reject
                  </p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="muted mt-1">
          Review applicants and assign a {PANEL_SIZE}-tutor interview panel (mark one head).
          Click a row to expand. The head schedules the interview and records the decision
