  ...
  return render(
    <NextIntlClientProvider locale="en" messages={en} timeZone="Asia/Shanghai">
      <ReadOnlyProvider value={readOnly}>
        <TutorDetailsButton tutorId={tutorId} name="Synthetic Tutor" />
      </ReadOnlyProvider>
...
it("opens an accessible person-scoped dialog on demand, with separate qualification and willingness", () => {
  ...
  expect(
    within(rows[0]!).getByText("Recorded grant from AP Computer Science A"),
  ).toBeTruthy();
...
it("shows an unlinked tutor without inventing membership or acceptance", () => {
  mocks.details.mockReturnValue({
    data: { ...detail, userId: null, badges: [], groups: [] },
  });
  show();
  open();
...
it("preserves historical details and clearly identifies revoked tutoring access", () => {
  mocks.details.mockReturnValue({
    data: { ...detail, tutorAccessRevoked: true, badges: [] },
  });
  show();
  open();
