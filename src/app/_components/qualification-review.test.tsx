...
vi.mock("~/trpc/react", () => ({
  api: {
    account: {
      me: {
        useQuery: () => ({
          data: { role: mocks.role, tutorId: mocks.tutorId },
        }),
      },
    },
    ...
    useUtils: () => ({
      qualificationApplication: { mine: { invalidate: vi.fn() } },
      subjectAvailability: { options: { invalidate: vi.fn() } },
      admin: { tutors: { invalidate: vi.fn() } },
    }),
  },
}));
const base = {
...
const show = (app = base) =>
  render(
    <NextIntlClientProvider locale="en" messages={messages}>
      <QualificationReview app={app} onChanged={vi.fn()} />
    </NextIntlClientProvider>,
  );

it.each(["ADMIN", "HEAD"])(
  "allows %s to decide after recording a note",
  (role) => {
    mocks.role = role;
    show();
...
it.each(["COORDINATOR", "VIEWER", "TUTOR"])(
  "hides decision controls for %s",
  (role) => {
    mocks.role = role;
    show();
    expect(
