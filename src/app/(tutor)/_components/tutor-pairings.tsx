...
export function TutorPairings() {
  ...
  const pendingByKey = new Map(
    (removalRequests.data ?? []).map((r) => [
      `${r.pairingId}:${r.tuteeId}`,
      r.id,
    ]),
  );
  ...
  return (
    <>
      <ul className="divide-y divide-slate-100">
                          ...
                          {pendingId ? (
                            <>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  recallRemoval.mutate({ requestId: pendingId })
                                }
                              >
        ...
        {setSlot.error && (
          <li className="py-2 text-sm text-red-600">{setSlot.error.message}</li>
        )}
      </ul>
