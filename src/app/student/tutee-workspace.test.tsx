...
const mocks = vi.hoisted(() => ({ personal: vi.fn(), requests: vi.fn(), view: "dashboard", settings: vi.fn() }));
vi.mock("next/navigation", () => ({ useSearchParams: () => new URLSearchParams({view: mocks.view}) }));
vi.mock("next-intl", () => ({
  useTranslations: () => (key: string) => key,
  useFormatter: () => ({ dateTime: () => "Sep 13" }),
}));
vi.mock("~/app/_components/confirm-dialog", () => ({ useDialog: () => ({ promptText: vi.fn(), dialog: null }) }));
vi.mock("~/trpc/react", () => ({ api: {
  student: {
    me: { useQuery: mocks.personal }, feedbackSettings: { useQuery: mocks.settings },
    appeal: { useMutation: () => ({ mutate: vi.fn() }) }, feedback: { useMutation: () => ({ mutate: vi.fn() }) },
  },
  studentWorkflow: { mine: { useQuery: mocks.requests } },
} }));
beforeEach(() => {
  mocks.view = "dashboard";
  mocks.personal.mockReturnValue({ data: { user: { name: "Sam" }, schedule: [], sessions: [], cards: [], appeals: [] } });
...
it("offers the existing form and account routes without requiring an enrollment", () => {
  ...
  expect(screen.getByRole("link", {name:"requestTutor"}).getAttribute("href")).toBe("/signup");
  expect(screen.getByText("formHelp")).toBeTruthy();
  expect(screen.getByRole("link", {name:/account accountHelp/}).getAttribute("href")).toBe("/my-account");
});
it("links to focused views with a single active navigation item and handles unknown deep links", () => {
  ...
  expect(screen.getByRole("link", {name:"attendance"}).getAttribute("aria-current")).toBe("page");
...
it("keeps unavailable data distinct from a genuine empty overview", () => {
  mocks.personal.mockReturnValue({ error: {message:"Connection interrupted"} });
  render(<TuteeOverview />);
...
it("shows schedules without unrelated attendance, disciplinary records or paging controls", () => {
  mocks.personal.mockReturnValue({ data: { schedule: [{id:"pair-1",subject:"Math",tutor:{englishName:"Tutor A"},dayOfWeek:1,startMin:930,endMin:990,room:null}], sessions: [], cards: [], appeals: [] } });
...
it("attendance pagination is not enabled by unrelated disciplinary history", () => {
  mocks.personal.mockReturnValue({ data: {schedule:[],sessions:[],cards:Array.from({length:20},()=>({id:"hidden-card"})),appeals:[]} });
...
it("support paging is independent of a full attendance page", () => {
  mocks.personal.mockReturnValue({ data: {schedule:[],sessions:Array.from({length:20},()=>({})),cards:[],appeals:[]} });
...
it("a full attendance page can move forward while retaining the selected section", () => {
  mocks.personal.mockReturnValue({ data: {schedule:[],sessions:Array.from({length:20},(_,i)=>({session:{id:String(i),date:new Date(),pairing:{subject:"Math"}},status:"PRESENT",feedback:null})),cards:[],appeals:[]} });
  render(<StudentPortal view="attendance" />);
