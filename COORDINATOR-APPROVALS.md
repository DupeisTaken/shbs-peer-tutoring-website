...
In **Management Actions** (`/admin/approvals`), ADMIN and HEAD can filter requests, inspect proposed values and the recorded state of affected records, and approve or reject with a required explanatory note. Coordinators can see and withdraw their own pending requests. Users cannot review their own requests even after promotion. Review results notify the requester and remain in their history.

The **My Approval Requests** coordinator banner opens the full history, including pending, approved, rejected and withdrawn proposals. Coordinators see only their own records. Reviewers start with pending requests and can filter by requester and status. Each card labels whether the proposal took effect, links to its detail, and retains submission/decision times, reviewer feedback or withdrawal attribution. **All Requests** returns from a detail to the full list; a detail link ignores prior filters and page offsets while preserving server ownership checks. Loading, empty/unavailable and error states remain visible, and Refresh reloads current outcomes.
